...
⚠️ **安全警告**: 使用旧盐值仅用于临时向后兼容，应尽快迁移数据！

1. 设置 `ENCRYPTION_SECRET` 环境变量
2. **临时措施**: 生成128字符的hex盐值（64字节）
   ```bash
   # 对于旧数据兼容（不推荐长期使用）
   # 注意：旧的硬编码 "salt" 太短，不符合64字节要求
   # 需要扩展到128个十六进制字符
   echo -n "salt" | xxd -p | head -c 128
   # 或生成新的安全盐值（推荐）
   openssl rand -hex 64
   ```
3. 设置 `ENCRYPTION_SALT=<128位hex字符串>`
4. **重要**: 尽快迁移所有加密数据到新的安全盐值
   - 解密旧数据
   - 使用新盐值重新加密
   - 更新 ENCRYPTION_SALT 环境变量
5. 定期轮换加密密钥和盐值
