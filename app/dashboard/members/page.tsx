/**
 * @file page.tsx
 * @description 会员管理页面 - 展示会员列表、统计信息、提供会员增删改查等功能
 * @author YYC³
 * @version 1.0.0
 * @created 2025-09-15
 */
...
export default function MembersPage() {
  ...
  const loadMembers = async () => {
    try {
      ...
      if (res.success && res.data) {
        ...
        toast({
          title: '加载失败',
          description: res.error || '无法加载会员数据',
          variant: 'destructive',
        })
      }
    } catch (error) {
      console.error('[v0] 加载会员数据失败:', error)
      setMembers([]) // 发生错误时也设置为空数组
      toast({
        title: '加载失败',
  ...
  const handleSubmitMember = async (data: MemberFormData) => {
    try {
      ...
      if (editingMember) {
        const res = await memberService.updateMember(editingMember.id, memberData)
        if (res.success) {
          toast({
            title: '更新成功',
            description: '会员信息已更新',
        ...
        const res = await memberService.createMember(memberData)
        if (res.success) {
          toast({
            title: '创建成功',
            description: '会员已创建',
  ...
  const filteredMembers = safeMembers.filter(member => {
    const matchesSearch =
      ...
      member.cardNo.includes(searchTerm)
  ...
  const stats = {
    ...
    newMembers: safeMembers.filter(m => {
      const createdDate = new Date(m.createdAt)
      const now = new Date()
      const monthAgo = new Date(now.getFullYear(), now.getMonth() - 1, now.getDate())
      return createdDate > monthAgo
    }).length,
    totalConsumption: safeMembers.reduce((sum, m) => sum + m.totalConsumption, 0),
    totalPoints: safeMembers.reduce((sum, m) => sum + m.points.total, 0),
  }
