...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (now - last < MIN_POLL_INTERVAL_MS) {
    ...
    return NextResponse.json(
      { error: "PollTooFrequent", message: `请降低轮询频率（≥${MIN_POLL_INTERVAL_MS}ms）`, retryAfterMs: MIN_POLL_INTERVAL_MS },
  ...
  try {
    ...
    const jitter = (Math.random() * 0.25) - 0.10;                       // -10% ~ +15% 抖动
    ...
    span.end(false);
    return NextResponse.json({ error: "ProgressFailed", message: error?.message ?? String(error) }, { status: 400 });
  }
}
