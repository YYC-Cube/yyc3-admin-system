...
test('UI 快照：主页关键组件', async ({ page }) => {
  ...
  await snap(page, 'stepbar', page.locator('[data-slot="stepbar"], [data-testid="stepbar"], div:has(button:has-text("1"))'))

  // InputPanel（Select + Textarea）
  await snap(page, 'input-panel', page.locator('[data-slot="input-panel"], [data-testid="input-panel"], textarea[data-slot="input-textarea"]'))

  // PreviewPanel（搜索框 + 按钮）
  await snap(page, 'preview-panel', page.locator('[data-slot="preview-panel"], [data-testid="preview-panel"], input[type="text"]'))
